...
interface ServingState {
  ...
  updateOrderStatus: (category: keyof OrdersData, table: string, orderId: number, status: string) => void;
  setServed: (category: keyof OrdersData, table: string, orderId: number, isServed: boolean) => void;
}
